...
import { isInternalConfiguration } from 'vs/base/common/nls';
...
function createDefaultConfiguration(): lp.NLSConfiguration {
	return {
		locale: 'en',
		availableLanguages: {},
	};
...
export async function getCachedNlsConfiguration(locale: string, userDataPath: string, commit?: string): Promise<lp.NLSConfiguration> {
	...
	if (!nlsConfiguration) {
		nlsConfiguration = new Promise(async resolve => {
			...
			if (commit && commit !== 'development' && (await exists(metadataPath))) {
				config = await lp.getNLSConfiguration(commit, userDataPath, metadataPath, locale);
			} else {
...
export const getTranslations = async (locale: string, userDataPath: string): Promise<Translations> => {
	const config = await getCachedNlsConfiguration(locale, userDataPath);
