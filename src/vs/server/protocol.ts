...
export class ServerProtocol extends PersistentProtocol {
	...
	public handshake(): Promise<ConnectionTypeRequest> {
		this.logger.info(this.logPrefix, '(Handshake 1/4)', 'Waiting for client authentication...');
			...
			const onControlMessageHandler = this.onControlMessage((rawMessage) => {
				try {
					...
					switch (message.type) {
						...
						case 'auth':
							this.logger.info(this.logPrefix, '(Handshake 2/4)', 'Client auth received!');
							this.sendMessage({ type: 'sign', data: message.auth });
							this.logger.info(this.logPrefix, '(Handshake 3/4)', 'Sent client signed auth...');
							break;
						case 'connectionType':
							cleanup();
							this.logger.info(this.logPrefix, '(Handshake 4/4)', 'Client has requested a connection!');
							resolve(message);
						default:
