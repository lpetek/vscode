...
import { main } from 'vs/server/cliProcessMain';
...
import { LocalReconnectConstants, TerminalSettingId } from 'vs/platform/terminal/common/terminal';
...
export class Vscode extends Disposable {
	...
	constructor () {
		...
		setUnexpectedErrorHandler((error) => {
			console.trace('Uncaught error', error instanceof Error ? error.message : error);
		});
	}
	...
	private async initializeServices(args: NativeParsedArgs): Promise<void> {
		...
		await new Promise<void>((resolve) => {
			...
			instantiationService.invokeFunction((accessor) => {
				...
				}, configurationService, logService, telemetryService);
				...
				this._register(combinedDisposable(instantiationService.createInstance(LogsDataCleaner), instantiationService.createInstance(ErrorTelemetry), this.ipc));
